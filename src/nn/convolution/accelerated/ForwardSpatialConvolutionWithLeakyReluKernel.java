 ...
 * @author bowen
 */
public class ForwardSpatialConvolutionWithLeakyReluKernel extends Kernel { //Sums the result of kernel depths, normal convolution
    ...
    public void call(float[] weights, int[] kernelSize, int[] kernelDim, int[] stride, int padding[], float[] input, int[] inputSize, int[] inputDim, float[] output, int[] outputSize, int[] outputDim, float leakiness) {
        ...
        for (int ii = 0; ii < kernelSize[0]; ii++) {
            for (int ij = 0; ij < kernelSize[1]; ij++) {
                for (int ik = 0; ik < kernelSize[2]; ik++) {
                    tempOutput = tempOutput + (getInput(ii + inputPosi, ij + inputPosj, ik, on) * getWeight(ii, ij, ik, ok));
                }
            }
        }
        tempOutput = tempOutput + getBiasWeight(ok);
        
        if (tempOutput >= 0) {
            output[outputIndex] = tempOutput;
        } else {
            output[outputIndex] = prop[0] * tempOutput;
        }
    }
}
